...
HIGHPRIO_TASKS_NOTIFICATION_INTERVAL = bot.config.phabricator.highpri_notify_interval
...
def searchphab(bot, channel, task=1):
    data = {
        'api.token': bot.config.phabricator.api_token,
        'constraints[ids][0]': task
    }
    response = requests.post(
        url='https://{0}/api/maniphest.search'.format(bot.config.phabricator.host),
        data=data)
    ...
    if go == 1:
        ...
        response2 = requests.post(
            url='https://{0}/api/user.search'.format(bot.config.phabricator.host),
            data=params)
        try:
...
def gethighpri(limit=True, channel=bot.config.highpri_channel, bot=None):
    data = {
        'api.token': bot.config.phabricator.api_token,
        'queryKey': bot.config.phabricator.querykey,  # mFzMevK.KRMZ for mhphab
    }
    response = requests.post(
        url='https://{0}/api/maniphest.search'.format(bot.config.phabricator.host),
        data=data)
        ...
        while x < len(data):
            ...
            if x > 5 and limit:
                bot.say("They are more than 5 tasks. Please see {0} for the rest or use .highpri".format(
                    bot.config.phabricator.host), channel)
                break
            else:
...
def high_priority_tasks_notification(bot):
    if bot.config.phabricator.highpri_notify == true:
