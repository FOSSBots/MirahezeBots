    ...
    if cont == 1:
        ...
        for line in file:
            ...
            if data[1] == wiki[0] and wiki[1] == data[2]:
                ...
                content = mwapi.main(request[0], str("User:" + (str(request[0]) + "/Status")), 
                                     "create", str("Updating status to " + str(request[1]) + " per " + str(request[0])), wikiurl, bot.settings.status.bot_username, bot.settings.status.bot_password, content=str(request[1]))
                return content
