...
def ghuser(instance: bot, message: trigger) -> None:
    ...
    try:
        instance.say(f'https://github.com/{quote(message.group(2))}')
    except TypeError:
...
def redditu(instance: bot, message: trigger) -> None:
    ...
    try:
        instance.say(f'https://reddit.com/u/{quote(message.group(2))}')
    except TypeError:
...
def redditr(instance: bot, message: trigger) -> None:
    ...
    try:
        instance.say(f'https://reddit.com/r/{quote(message.group(2))}')
    except TypeError:
...
def wmca(instance: bot, message: trigger) -> None:
    ...
    try:
        instance.say(
            f'https://meta.wikimedia.org/wiki/Special:CentralAuth/{quote(message.group(2).replace(" ", "_"))}',
            )
...
def mhca(instance: bot, message: trigger) -> None:
    ...
    try:
        instance.say(
            f'https://meta.miraheze.org/wiki/Special:CentralAuth/{quote(message.group(2).replace(" ", "_"))}',
            )
    except AttributeError:
...
def twlink(instance: bot, message: trigger) -> None:
    ...
    try:
        instance.say(f'https://twitter.com/{quote(message.group(2))}')
    except TypeError:
...
def mhwiki(instance: bot, message: trigger) -> None:
    ...
    try:
        ...
        if len(options) == 1:
            instance.say(
                f'https://meta.miraheze.org/wiki/{quote(options[0].replace(" ", "_"))}',
                )
        elif len(options) == 2:
            wiki = options[0]
            page = quote(options[1].replace(' ', '_'))
